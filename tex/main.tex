...
is a little difficult to solve ($\sim$\SI{1}{h} $\times$ \SI{2}{Gflop/s}),
so we focus here on
\begin{quote}
	$n = 100$ requests
	for
...
whilst
the fleet of minibuses ($C = 8$) can handle most requests,
see Fig.~\ref{f:case1-hist}.

%

The vehicle load over time is shown in Fig.~\ref{f:case1-load},
indicating that it runs at about half the capacity.
%
Since all $n = 100$ pickup requests are from the first \SI{17}{min} only,
the fleet would be able to handle more requests per minute
if operating over an extended period.
...
\begin{figure}[!p]
	...
	\caption{%
		Passenger trip trajectories
		colored by excess travel time
		w.r.t.~quickest route
		($n = 100$ requests for $N = 10$ vehicles).
		%
		%
		Empty red circles are unserviced requests.
		%
		See \S\ref{s:case1}.
	}
	%
